// Helpers de "telas" (product.product) compartidos entre /api/odoo/productos
// (búsqueda libre) y /api/odoo/articulos (productos de las OC del expediente):
// extraen de un producto real de Odoo los datos que la etiqueta necesita.
...
export function nombreCortoDesdeNombre(nombre) {
...
export function composicionDesdeNombre(nombre) {
...
// Enriquece productos crudos de Odoo (con id, default_code, name, tipo,
// attribute_value_ids) al shape que consume el editor de artículos:
// { id, codigo, nombre, nombreCompleto, composicion, color }. El color legible
// se resuelve en UNA sola consulta de product.attribute.value.
export async function enriquecerProductos(productos) {
